...
**In progress.** The first implementation now provides a deterministic campaign geography query with prototype terrain, elevation, slope, and drainage plus historically sourced, date-enabled mining districts at Arditurri and Irugurutzeta. Etxe, field, and mine placement use distinct suitability rules, share the normal work-radius constraint, and expose transparent green/red previews with reasons. The non-mining terrain values and deposit radii remain prototype campaign abstractions; researched spatial data, fertility, rivers/flooding, routes, access rights, construction difficulty, and productivity modifiers remain to be added.
